document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateStepDisplay() {
    steps.forEach((s) => {
      s.classList.toggle(
        "step-content--hidden",
        parseInt(s.dataset.step) !== currentStep
      );
    });
    ...
    nextBtn.textContent =
      currentStep === totalSteps ? "Опубликовать" : "Дальше";
  ...
  openModalBtns.forEach((b) =>
    b.addEventListener("click", () => modal.classList.add("modal--active"))
  );
  closeElements.forEach((c) =>
    c.addEventListener("click", (e) => {
      if (e.target === c || c.classList.contains("modal__btn-primary"))
    ...
    photoInput.addEventListener("change", (e) => {
      ...
      files.forEach((file) => {
        ...
        reader.onload = (event) => {
          ...
          item
            .querySelector(".gallery-item__remove")
    ...
    docInput.addEventListener("change", (e) => {
      Array.from(e.target.files).forEach((file) => {
        ...
        docItem
          ...
          .addEventListener("click", () => docItem.remove());
      });
    });

    ["dragenter", "dragover"].forEach((name) => {
      docArea.addEventListener(name, () =>
        docArea.classList.add("upload-area--dragover")
      );
    });
    ["dragleave", "drop"].forEach((name) => {
      docArea.addEventListener(name, () =>
        docArea.classList.remove("upload-area--dragover")
      );
    });
  }
});
